...
class DataPrefetcher():
    ...
    def preload(self):
        ...
        with torch.cuda.stream(self.stream):
            for k in self.batch:
                if k != 'meta':
                    if self.batch[k].dtype == '<U6':
                        self.batch[k] = self.batch[k].astype(np.float64)
                    self.batch[k] = torch.from_numpy(self.batch[k]).to(device=self.device, non_blocking=True)
...
def eval_one_epoch(cfg, model, dataloader, epoch_id, logger, dist_test=False, save_to_file=False, result_dir=None):
    ...
    for i in range(0,1):
        ...
        end_time = time.time()
        print('The speed of test is', len(dataloader.dataset) / (end_time - start_time), 'FPS')

...
class PointHeadBox(PointHeadTemplate):
    ...
    def assign_targets(self, input_dict):
        ...
        """
        point_coords = input_dict['points']
    ...
    def forward(self, batch_dict):
        ...
        x = point_features
        for cls in self.cls_layers:
            if 'Batch' in str(cls.type):
                x = x.transpose(-1,-2)
                x = cls(x)
                x = x.transpose(-1, -2)
            else:
        ...
        point_cls_preds = x
        x = point_features
        for box in self.box_layers:
            if 'Batch' in str(box.type):
                x = x.transpose(-1, -2)
                x = box(x)
                x = x.transpose(-1, -2)
            else:
       ...
       # point_box_preds = self.box_layers(point_features)  # (total_points, box_code_size)
        ...
        if not self.training or self.predict_boxes_when_training:
            points = batch_dict['point_coords'][:, 1:4].view((batch_dict['batch_size'],-1,3))

...
class PointRCNNHead(RoIHeadTemplate):
    def __init__(self, input_channels, model_cfg, num_class=1):
        ...
        self.merge_down_layer = nn.Sequential(
            nn.Conv2d(256, c_out, kernel_size=1, bias=not use_bn),
        ...
        for k in range(self.model_cfg.SA_CONFIG.NPOINTS.__len__()):
            ...
            mlps = self.model_cfg.SA_CONFIG.MLPS[k]
    ...
    def roipool3d_gpu(self, batch_dict):
        ...
        point_scores = point_scores.view(batch_size,-1,1)

        point_depths = point_coords.norm(dim=1) / self.model_cfg.ROI_POINT_POOL.DEPTH_NORMALIZER - 0.5
        point_depths = point_depths.view(batch_size, -1, 1)

        point_features_list = [point_scores[:, None], point_depths[:, None], point_features[:,None]]

...
class Detector3DTemplate(nn.Module):
    ...
    def post_processing(self, batch_dict):
        ...
        #print('post process',time.time()-t1)
        t0 = time.time()
        '''        
        path = '/media/ddd/data2/3d_MOTS_Ex./Code/OpenPCDet-RandlaNet/tools/time/'
        if os.path.exists(path+'postprocess.npy'):
        	t = np.load(path+'postprocess.npy')
        	t =list(t)
        	t.append(t0-t1)
        	np.save(path+'postprocess.npy',t)
        else:
        	np.save(path+'postprocess.npy',[t0-t1])
        '''         

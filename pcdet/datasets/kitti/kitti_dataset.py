...
class KittiDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None,use_color=False,use_rgb=False,\
                 nbg=True,raw=True):
        """
        Args:
            root_path:
        ...
        self.use_rgb = use_rgb # False--lab True--rgb
        self.nbg = nbg
        self.raw = raw
        self.kitti_infos = []
    ...
    def get_lidar_raw(self, idx):
        ...
        lidar_file = self.root_split_path / 'velodyne_m_vfe' / ('%s.npy' % idx)
    ...
    def evaluation(self, det_annos, class_names,eval_distance=False, **kwargs):

        if 'annos' not in self.kitti_infos[0].keys():
            return None, {}

        from .kitti_object_eval_python import eval as kitti_eval
        if not eval_distance:
            ...
            for info in det_annos:
                ...
                for i in range(loc.shape[0]):
                    dist = np.sqrt(np.sum(loc[i] ** 2))
                    #print(np.max(loc[i]),np.min(loc[i]))
                    if dist <= 60:
                        # info['name'][i] = 'DontCare'
                        index_set.append(i)
                for k, v in info.items():
                    if k == 'frame_id':
                        det.update({k:v})
                    else:
                        det.update({k: v[index_set]})
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = KittiDataset(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False,raw=True)
